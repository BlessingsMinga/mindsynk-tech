...
Work Log:
- Added a `whatsapp` field ("265992092766") to `companyInfo` in `src/lib/data.ts` so the number is centralized and reusable.
...
- Clicking it opens a WhatsApp chat with the company number (265992092766) pre-filled with "Hi MindSynk Technologies, I'd like to discuss a project."
